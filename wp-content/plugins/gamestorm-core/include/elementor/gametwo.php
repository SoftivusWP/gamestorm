 ...
 * @since 1.0.0
 */
class TP_Gametwo extends Widget_Base
{
    ...
    protected function register_controls()
    {
        ...
        $this->start_controls_section(
            'gameone_heading_general_content_two',
            [
    ...
    protected function render()
    {
        ...
        <script>
            ...
            jQuery('.gametwoo button').on('click', function() {
                // Remove active class from all buttons
                jQuery('.gametwoo button').removeClass('active');
                ...
                jQuery('.tt').removeClass('active');
                jQuery('.tt[data-category="' + category + '"]').addClass('active');
            });

            // Simulate a click on the first button to set the initial state
            jQuery('.gametwoo button:first').trigger('click');

            // Event handler for button click
            jQuery('.gametwoo button').on('click', function(event) {
        ...
        <section class="our-games overflow-hidden index-two position-relative pt-120 pb-120">
            <?php if (!empty($settings['gametwo_reborn_show'] == 'yes')) :   ?>
                <div class="shape-area">
                    <img src="<?php echo get_template_directory_uri() ?>/assets/img/abs-items/our-games-shape-1.png" class="shape-1" alt="icon">
                    <img src="<?php echo get_template_directory_uri() ?>/assets/img/abs-items/our-games-shape-2.png" class="shape-2" alt="icon">
                    <img src="<?php echo get_template_directory_uri() ?>/assets/img/abs-items/our-games-shape-3.png" class="shape-3" alt="icon">
                    <img src="<?php echo get_template_directory_uri() ?>/assets/img/abs-items/our-games-shape-4.png" class="shape-4" alt="icon">
                </div>
            <?php endif ?>

            <div class="container singletab position-relative cus-z1">
                ...
                <div class="row justify-content-center">
                    <div class="col-xl-6 col-lg-7 text-center">
                        ...
                        <?php
                                ...
                                if ($categories) :
                                    ?>
                            <ul class="nav tablinks  gametwoo flex-wrap d-center mb-6 mb-sm-10 d-inline-flex gap-4 p-3 tab-area">
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-12">
                        <div class="tabcontents visible-from-bottom">
                            <?php foreach ($categories as $index => $category) : ?>
                                <?php
                                            $args = array(
                                                'post_type' => 'games',
                                                ...
                                                'offset'         => 0,
                                                'post_status'    => 'publish',
                                                'post__in'          => $settings['game_two_category'],
                                            );
                                            ...
                                            if ($posts) :
                                                ...
                                                $column_count = 2; // Number of columns per row

                                                for ($i = 0; $i < $post_count; $i += $column_count) :
                                                    ?>
                                        <div class="row  game-container">
                                            <?php for ($j = $i; $j < $i + $column_count && $j < $post_count; $j++) : ?>
                                                <?php $current_post = $posts[$j]; ?>
                                                <div class="col-md-6">
                                                    <div class="tabitem tt <?php echo $index === 0 && $j === $i ? 'active' : ''; ?>" data-category="<?php echo esc_attr($category->slug); ?>">
                                                        <!-- Your game post content here -->
                                                        <!-- ... (your existing post content code) ... -->

                                                        <?php
                                                                                $game_rating = function_exists('get_field') ? get_field('total_rating', $current_post->ID) : '4.5';
                                                                                $game_review = function_exists('get_field') ? get_field('total_review', $current_post->ID) : '5.5M Riview';
                                                                                $game_download = function_exists('get_field') ? get_field('total_download', $current_post->ID) : '500M+';
                                                                                $game_download_text = function_exists('get_field') ? get_field('download_text', $current_post->ID) : 'Downloads';
                                                                                $game_feature_title = function_exists('get_field') ? get_field('feature_tag_title', $current_post->ID) : 'Feature';
                                                                                $apple_image = function_exists('get_field') ? get_field('apple_store_image', $current_post->ID) : '';
                                                                                $apple_link = function_exists('get_field') ? get_field('apple_store_link', $current_post->ID) : '';
                                                                                $play_image = function_exists('get_field') ? get_field('play_store_image', $current_post->ID) : '';
                                                                                $play_link = function_exists('get_field') ? get_field('play_store_link', $current_post->ID) : '';
                                                                                $amazon_image = function_exists('get_field') ? get_field('amazon_store_image', $current_post->ID) : '';
                                                                                $amazon_link = function_exists('get_field') ? get_field('amazon_store_link', $current_post->ID) : '';
                                                                                $video_link = function_exists('get_field') ? get_field('game_video_link', $current_post->ID) : '';
                                                                                $game_thumbnail = function_exists('get_field') ? get_field('thumbnail_image_game', $current_post->ID) : '';
                                                        ...
                                                        <div class="single-box">
                                                            <div class="position-relative d-center">
                                                                <?php if (!empty($settings['gameone_feature_show'])) :   ?>
                                                                ...
                                                                <?php if (has_post_thumbnail($current_post->ID)) : ?>
                                                                    <?php echo get_the_post_thumbnail($current_post->ID, 'gametwo', array('class' => 'w-100 thumb-img')); ?>
                                                                <?php endif; ?>
                                                            ...
                                                            <div class="info-area position-relative p-3 p-lg-5">
                                                                <div class="d-flex align-items-end gap-4 gap-sm-8">
                                                                    ...
                                                                    <div class="info-area">
                                                                        <a href="<?php the_permalink($current_post->ID) ?>">
                                                                            <h3 class="visible-slowly-bottom mb-4"><?php echo esc_html(get_the_title($current_post->ID)); ?></h3>
                                                                        </a>
                                                                        <?php
                                                                                                // Get the terms for the 'game_genre' taxonomy
                                                                                                $game_genres = get_the_terms($current_post->ID, 'games-cat');
                                                                ...
                                                                <?php if (!empty($settings['game_one_review_area'])) :   ?>
                                                                ...
                                                                <?php endif ?>
                                                                <?php if (!empty($settings['gameone_link_show'])) :   ?>
...
$widgets_manager->register(new TP_Gametwo());
